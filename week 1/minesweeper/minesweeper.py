...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        # If the number of mines is equal to the number of cells in the sentence, then all cells are mines
    ...
    def mark_safe(self, cell):
        ...
        """
        # if cell is in sentence, remove it
        if cell in self.cells:
            self.cells.remove(cell)
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width and (i, j) not in self.safes and (i, j) not in self.mines:
                    undiscoverded_cells.append((i, j))

        new_sentence = Sentence(undiscoverded_cells, count - mine_count)
        ...
        for sentence in self.knowledge:
            if new_sentence.cells.issubset(sentence.cells) and new_sentence.count > 0 and sentence.count > 0 and new_sentence != sentence:
                new_sentence_cells = sentence.cells - new_sentence.cells
                new_sentence_count = sentence.count - new_sentence.count
